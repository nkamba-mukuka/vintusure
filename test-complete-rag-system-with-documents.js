...
const queryDocumentsRAG = httpsCallable(functions, 'queryDocumentsRAG');
    ...
    try {
        ...
        if (result.data.success) {
            const countField = systemName === 'Customer' ? 'similarCustomersCount' :
                               systemName === 'Claims' ? 'similarClaimsCount' : 
                               systemName === 'Policies' ? 'similarPoliciesCount' : 'similarDocumentsCount';
            ...
            if (result.data.sources?.length > 0) {
                ...
                result.data.sources.forEach((source, i) => {
                    const nameField = systemName === 'Customer' ? 'customerName' :
                                    systemName === 'Claims' ? 'claimDescription' : 
                                    systemName === 'Policies' ? 'policyNumber' : 'fileName';
...
async function testDocumentsRAG() {
    console.log('\n🚀 Testing Documents RAG System...');
    await testRAG("Find insurance policy documents", queryDocumentsRAG, 'Documents');
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testRAG("Show me claim-related files", queryDocumentsRAG, 'Documents');
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testRAG("Documents about vehicle coverage", queryDocumentsRAG, 'Documents');
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testRAG("Find PDF files with contract information", queryDocumentsRAG, 'Documents');
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testRAG("Show me recent invoice documents", queryDocumentsRAG, 'Documents');
    await new Promise(resolve => setTimeout(resolve, 1000));
}

// Main test execution
(async () => {
    console.log('🧪 Testing Complete VintuSure RAG System with Documents...\n');
    console.log('📊 Testing all four entities: Customers, Claims, Policies, and Documents');
    ...
    console.log('📁 Documents RAG enables semantic search across uploaded files\n');

    // Test all four RAG systems
    await testCustomerRAG();
    await testClaimsRAG();
    await testPoliciesRAG();
    await testDocumentsRAG();
    ...
    console.log('✅ Documents RAG: Semantic search across uploaded files');
    ...
    console.log('✅ File Processing: Documents support text extraction and metadata');
    console.log('✅ Automatic Indexing: All entities are indexed on creation');
})();
