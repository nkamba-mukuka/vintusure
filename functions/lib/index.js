...
exports.indexCustomerData = (0, firestore_2.onDocumentCreated)({
...
exports.indexClaimData = (0, firestore_2.onDocumentCreated)({
...
exports.indexPolicyData = (0, firestore_2.onDocumentCreated)({
...
exports.indexDocumentData = (0, firestore_2.onDocumentCreated)({
...
function createClaimEmbeddingText(claim) {
    return [
        `Claim ID: ${claim.id}`,
...
function createDocumentEmbeddingText(document) {
    ...
    if (document.metadata) {
        if (document.metadata.author)
            baseText.push(`Author: ${document.metadata.author}`);
        if (document.metadata.subject)
            baseText.push(`Subject: ${document.metadata.subject}`);
        if (document.metadata.keywords?.length)
            baseText.push(`Keywords: ${document.metadata.keywords.join(', ')}`);
        if (document.metadata.language)
            baseText.push(`Language: ${document.metadata.language}`);
        if (document.metadata.pageCount)
            baseText.push(`Pages: ${document.metadata.pageCount}`);
    }
    return baseText.join('. ');
}
// Generate embedding using Vertex AI Embeddings API
async function generateCustomerEmbedding(text) {
    try {
        const project = 'vintusure';
        const location = 'us-central1';
...
exports.queryCustomerRAG = (0, https_1.onCall)({
    memory: '1GiB',
    timeoutSeconds: 120,
    maxInstances: 10,
    cors: [
...
exports.queryClaimsRAG = (0, https_1.onCall)({
    memory: '1GiB',
    timeoutSeconds: 120,
    maxInstances: 10,
    cors: [
...
exports.queryPoliciesRAG = (0, https_1.onCall)({
    memory: '1GiB',
    timeoutSeconds: 120,
    maxInstances: 10,
    cors: [
...
exports.queryDocumentsRAG = (0, https_1.onCall)({
    memory: '1GiB',
    timeoutSeconds: 120,
    maxInstances: 10,
    cors: [
...
async function searchCustomerVectors(queryEmbedding, topK = 5) {
    try {
        const project = 'vintusure';
        const location = 'us-central1';
...
async function getCustomerContexts(similarCustomers) {
    try {
...
async function searchClaimVectors(queryEmbedding, topK = 5) {
    try {
        const project = 'vintusure';
        const location = 'us-central1';
...
async function searchDocumentVectors(queryEmbedding, topK = 5) {
    try {
        const project = 'vintusure';
        const location = 'us-central1';
        ...
        const neighbors = result.nearestNeighbors?.[0]?.neighbors || [];
        const searchResults = neighbors.map((neighbor) => {
...
async function searchPolicyVectors(queryEmbedding, topK = 5) {
    try {
        const project = 'vintusure';
        const location = 'us-central1';
        const indexEndpointId = '7427247225115246592';
...
async function getClaimContexts(similarClaims) {
    try {
...
async function generateRAGResponse(query, customerContexts, userId) {
    try {
        ...
        const contextText = customerContexts.map((ctx, index) => `Customer ${index + 1} (Similarity: ${(ctx.similarity * 100).toFixed(1)}%):\n${ctx.context}`).join('\n\n');
        // Create RAG prompt
...
async function generateClaimsRAGResponse(query, claimContexts, userId) {
    try {
        console.log(`Generating Claims RAG response for query: "${query}"`);
        // Prepare context from similar claims
        const contextText = claimContexts.map((ctx, index) => `Claim ${index + 1} (Similarity: ${(ctx.similarity * 100).toFixed(1)}%):\n${ctx.context}`).join('\n\n');
        // Create RAG prompt
...
async function generatePoliciesRAGResponse(query, policyContexts, userId) {
    try {
        console.log(`Generating Policies RAG response for query: "${query}"`);
        // Prepare context from similar policies
        const contextText = policyContexts.map((ctx, index) => `Policy ${index + 1} (Similarity: ${(ctx.similarity * 100).toFixed(1)}%):\n${ctx.context}`).join('\n\n');
        // Create RAG prompt
...
async function getDocumentContexts(searchResults) {
    try {
...
async function generateDocumentsRAGResponse(query, documentContexts, userId) {
    try {
