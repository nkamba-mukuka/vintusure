...
type ProfileOnboardingFormData = z.infer<typeof profileOnboardingSchema>;
...
export default function ProfileOnboarding({ onComplete }: ProfileOnboardingProps) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<ProfileOnboardingFormData>({
  ...
  const onSubmit = async (data: ProfileOnboardingFormData) => {
    if (!user?.uid) {
      toast({
        title: 'Error',
    ...
    try {
      ...
      // Call the completion callback or navigate to dashboard
      if (onComplete) {
        onComplete();
      } else {
        navigate('/dashboard');
